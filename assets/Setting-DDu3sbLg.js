import{u as E,t as z,r as v,k as e,o as w,j as t,q as A}from"./index-DxluskjS.js";import{c as j,b as h,a as F,u as U}from"./index.esm-DHY6pluF.js";const G=j({UserName:h().required("نام کاربری الزامی است").min(3,"باید حداقل ۳ کاراکتر باشد").max(20,"نباید بیشتر از ۲۰ کاراکتر باشد"),UserAge:F().typeError("سن باید عدد باشد").min(1,"سن باید بیشتر از ۰ باشد").max(120,"سن معتبر نیست"),UserCountry:h()}),$=j({UserPassword:h().min(6,"باید بیشتر از 6 کاراکتر باشد").max(20,"نباید بیشتر از 20 کاراکتر باشد"),UserEmail:h().email("ایمیل معتبر نیست").required("ایمیل الزامی است"),UserPhone:h().matches(/^[0-9+-\s()]*$/,"شماره تلفن معتبر نیست")}),T=j({FavoritesTopic:h().min(3,"باید حداقل ۳ کاراکتر باشد").max(50,"نباید بیشتر از ۵۰ کاراکتر باشد"),Gender:h().required("جنسیت الزامی است")}),q=t.div`
  width: 100%;
  height: 80vh;
  margin: 0 auto;
  display: flex;
